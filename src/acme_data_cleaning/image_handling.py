...
class InterpolatingResampler:
    ...
    def resample(self, images, masks=None):
        ...
        reshaped_input = convolved_images.reshape([n_leading_channels, 1] + list(final_dimensions))

        # This doesn't actually touch the underlying memory, so it should be a pretty efficient approach.
        ...
        # Now we reshape the data and locations, to make pytorch happy
        reshaped_masks = convolved_masks.reshape([n_leading_channels, 1] + list(final_dimensions))
...
def make_resampler(metadata, config, dummy_im):
    ...
    if config['interpolate']:
        ...
        # output_pixel_size
        binning_factor = (
                    wavelength * det_distance / (output_shape[0] * pixel_pitch * (config['output_pixel_size'] * 1e-9)))
